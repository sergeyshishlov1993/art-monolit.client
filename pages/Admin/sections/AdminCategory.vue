...
async function getData() {
  const firestoreQuery = query(
    collection(db, `product/catalog/${activeTab.value}`),
    orderBy("timestamp"),
    limit(8)
  );
...
async function removeCard(id, path) {
  const documentRef = doc(db, `product/catalog/${activeTab.value}`, id);
...
async function updateDocumentById(date, documentId, url, newImg) {
  ...
  try {
    const docRef = doc(db, `product/catalog/${activeTab.value}`, documentId);
